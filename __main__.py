...
from app import AISwinUnetrSegApp
...
if __name__ == "__main__":
    ...
    app = AISwinUnetrSegApp(do_run=False)
    ...
    logging.info(f'Start processing input in: {app_input_path} with results in: {app_output_path}')
    idx = 0
    work_dir = f".swin_unetr_app_workdir{idx}"
    app.run(input=app_input_path, output=app_output_path, workdir=work_dir)
    logging.info(f"Completed processing input in: {app_input_path} with results in: {app_output_path}")
